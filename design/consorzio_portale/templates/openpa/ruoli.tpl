<div class="global-view-full">
    ...
    {def $nomi = array( 'Responsabile', 'Referente', 'Direttore' )
    ...
    {def $schede = fetch( 'content', 'tree', hash( 'parent_node_id', 1,
                                                  'main_node_only', true(),
                                                  'class_filter_type', 'include',
                                                  'class_filter_array', array( 'scheda_informativa' ),
                                                  'sort_by', array( 'name', true() ) ) )
         $progetti = fetch( 'content', 'tree', hash( 'parent_node_id', 1,
                                                    'main_node_only', true(),
                                                    'class_filter_type', 'include',
                                                    'class_filter_array', array( 'progetto' ),
                                                    'sort_by', array( 'name', true() ) ) )}
    ...
    <div class="block">
        ...
        <p>
            ...
            <select name="Struttura">
                ...
                <optgroup label="Servizi offerti">
                {foreach $schede as $item}
                    <option value="{$item.contentobject_id}">{$item.name|wash()} ({$item.parent.parent.name|wash()}/{$item.parent.name|wash()})</option>
                {/foreach}
                </optgroup>
                <optgroup label="Progetti">
                {foreach $progetti as $item}
                    <option value="{$item.contentobject_id}">{$item.name|wash()} ({$item.parent.parent.name|wash()}/{$item.parent.name|wash()})</option>
                {/foreach}
                </optgroup>
            </select>
    ...
    <table width="100%" cellspacing="0" cellpadding="0" border="0" class="list">
        ...
        <tbody>
            ...
            <tr class="{$style}">
                ...
                <td width="1"><div class="listbutton"><a href={concat("content/edit/",$role.contentobject_id,"/f/ita-IT")|ezurl}><img class="" src={"edit.gif"|ezimage} width="16" height="16" alt="Edit" /></a></div></td>
                <td width="1">
