{if is_area_tematica()}
    ...
    {elseif and( is_set( $area.data_map.image ), $area.data_map.image.has_content, $area.data_map.image.content['original'].height|gt(200))}
        <div class="head-banner hidden-xs" style="background-image: url({$area.data_map.image.content['header_banner'].url|ezroot(no)});"></div>
    {/if}
    {undef $area}
