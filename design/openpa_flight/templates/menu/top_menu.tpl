<div class="main-nav" role="navigation">
    ...
    {else}
        ...
        <ul class="horizontal_list main_menu clearfix">
          ...
          {def $top_menu_node_ids_count = $top_menu_node_ids|count()}{if $top_menu_node_ids_count}
          {foreach $top_menu_node_ids as $id}
            ...
            {if $tree_menu.has_children}
              {if $tree_menu.max_recursion|eq(1)}
                <div class="sub_menu_wrap"><ul class="sub_menu">{foreach $tree_menu.children as $child}<li>{include name="top_sub_menu" uri='design:menu/top_menu_item.tpl' menu_item=$child}</li>{/foreach}</ul></div>
              {else}
                ...
                {foreach $tree_menu.children as $child}
                  ...
                  {if $child.has_children}<ul class="sub_menu">{foreach $child.children as $sub_child}<li>{include name="top_sub_menu" uri='design:menu/top_menu_item.tpl' menu_item=$sub_child}</li>{/foreach}</ul>{/if}
                </div>
                {/foreach}
                </div>
              {/if}
