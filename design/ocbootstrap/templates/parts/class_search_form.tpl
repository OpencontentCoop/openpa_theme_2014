...
{if count($facetQuery)|gt(0)}
  ...
  {/if}
  {def $searchForFacets = api_search( concat( 'classes ', $class.identifier, ' facets [', $facetQuery|implode(','), ']', $subtreeQuery, 'limit 1' ) )}  
  {if is_set($searchForFacets.facets)}
	{foreach $searchForFacets.facets as $facetResult}
	  {if count($facetResult.data)|gt(0)}				
		{set $availableValues = $availableValues|merge( hash( $facetResult.name|explode('.id')|implode(''), $facetResult.data ) )}
	  {/if}
	{/foreach}
  {/if}
{/if}
...
<form id="search-form-box" action="{'content/advancedsearch'|ezurl('no')}" method="get">
  ...
  <fieldset>
	...
	{foreach $subtree as $value}
	  <input name="Subtree[]" type="hidden" value="{$value|wash()}"/>
	{/foreach}
	
	<input name="Class" type="hidden" value="{$class.identifier|wash()}"/>
